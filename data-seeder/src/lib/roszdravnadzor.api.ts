import axios from "axios"
import "dotenv/config"
import qs from "qs"
import slugify from "slugify"

const API_TOKEN = process.env.API_TOKEN
const API_URL = process.env.API_URL


const headers = {
  "Content-Type": "application/json",
  Authorization: `Bearer ${API_TOKEN}`,
}

export const filldb = async (posts) => {
  if (posts[0].category) return console.error("Error consistent posts data.")
  
  const parents = []
  const mem = new Map()
  let index = 0
  let prevlevel = null
  
  try {
    for await (const post of posts) {
      if (!post.category) continue
      if (
        post.category &&
        post.category !== parents?.[parents.length - 1 || 0]?.[1]
      ) {
        if (parents.length === 0 || parents?.[0]?.[1] != post.section) {
          const slug = slugify(post.section_name, { lower: true, strict: true })
          const body = {
            data: {
              title: post.section_name,
              code: post.section,
              slug,
            },
          }
          
          const response = await axios({
            url: `/api/categories`,
            method: "POST",
            data: body,
            headers,
            baseURL: API_URL,
          }).then(async (value) => {
            console.log(
              `${index}. Create category with documentId ${value.data.data.documentId} and add to database`,
            )
            
            return value
          }).catch(async () => {
            const query = qs.stringify(
              {
                fields: ["id", "documentId", "code"],
                filters: {
                  slug,
                },
              },
              {
                encodeValuesOnly: true,
              },
            )
            
            const response = await axios({
              url: `/api/categories`,
              method: "GET",
              params: new URLSearchParams(query),
              headers,
              baseURL: API_URL,
            })
            
            if (!response.data.data[0]) return null
            
            console.log(
              `${index}. Get category with documentId ${response.data.data[0].documentId} from database`,
            )
            
            return {
              data: {
                data: response.data.data[0],
              },
            }
          })
          
          parents[0] = [
            response?.data.data.documentId,
            response?.data.data.code,
          ]
        }
        
        const level = spcode(post.category || post.section)
        
        const slug = slugify(post.category_name, { lower: true, strict: true })
        const body = {
          data: {
            title: post.category_name,
            code: post.category,
            slug,
          },
        }
        
        if (post.category && prevlevel === level) {
          body.data.description = post.description
          body.data.parents = { connect: [parents[parents.length - 2 || 0][0]] }
        } else if (post.category && prevlevel !== level) {
          body.data.description = post.description
          body.data.parents = { connect: [parents[parents.length - 1][0]] }
        }
        
        prevlevel = level
        
        const response = await axios({
          url: `/api/categories`,
          method: "POST",
          data: body,
          headers,
          baseURL: API_URL,
        }).then(async (value) => {
          console.log(
            `${index}. Create category with documentId ${value.data.data.documentId} with parent ${body.data.parent} and add to database`,
          )
          
          return value
        }).catch(async (e) => {
          console.log(e.response)
          const query = qs.stringify(
            {
              fields: ["id", "documentId", "code"],
              filters: {
                slug:
                  slug ||
                  slugify(post.section_name, { lower: true, strict: true }),
              },
            },
            {
              encodeValuesOnly: true,
            },
          )
          
          const response = await axios({
            url: `/api/categories`,
            method: "GET",
            params: new URLSearchParams(query),
            headers,
            baseURL: API_URL,
          })
          
          if (!response.data.data[0]) return null
          
          console.log(
            `${index}. Get category with documentId ${response.data.data[0].documentId} from database`,
          )
          
          return {
            data: {
              data: response.data.data[0],
            },
          }
        })
        
        if (!response?.data.data) {
          return console.error(
            `${index}. Error creating or getting category data.`,
          )
        }
        const data = [response?.data.data.documentId, response?.data.data.code]
        
        parents.splice(level, parents.length - level, data)
      }
      if (mem.has(post.name)) {
        const body = {
          data: {
            parents: { connect: [...parents].map((parent) => parent[0]) },
          },
        }
        await axios({
          url: `/api/categories/${mem.get(post.name)}`,
          method: "PUT",
          data: body,
          headers,
          baseURL: API_URL,
        }).then(async (value) => {
          console.log(
            `${index}. Updated category with documentId ${value.data.data.documentId} and add parent ${parents[parents.length - 1][0]}`,
          )
          return value
        })
      } else {
        const slug = slugify(post.name, { lower: true, strict: true })
        const body = {
          data: {
            title: post.name,
            code: post.code,
            description: post.description,
            parents: { connect: [...parents].map((parent) => parent[0]) },
            slug,
          },
        }
        const response = await axios({
          url: `/api/categories`,
          method: "POST",
          data: body,
          headers,
          baseURL: API_URL,
        }).then(async (value) => {
          console.log(
            `${index}. Create category with documentId ${value.data.data.documentId} and add to database`,
          )
          return value
        }).catch(async () => {
          const query = qs.stringify(
            {
              fields: ["id", "documentId", "code"],
              filters: {
                slug,
              },
            },
            {
              encodeValuesOnly: true,
            },
          )
          
          const response = await axios({
            url: `/api/categories`,
            method: "GET",
            params: new URLSearchParams(query),
            headers,
            baseURL: API_URL,
          })
          
          if (!response.data.data[0]) return null
          
          console.log(
            `${index}. Get category with documentId ${response.data.data[0].documentId} from database`,
          )
          
          const body = {
            data: {
              parents: {
                connect: [...parents].map((parent) => parent[0]),
              },
            },
          }
          await axios({
            url: `/api/categories/${response.data.data[0].documentId}`,
            method: "PUT",
            data: body,
            headers,
            baseURL: API_URL,
          }).then(async (value) => {
            console.log(
              `${index}. Updated category with documentId ${value.data.data.documentId} and add parent ${parents[parents.length - 1][0]}`,
            )
            return value
          })
          
          return {
            data: {
              data: response.data.data[0],
            },
          }
        })
        if (!response?.data.data) {
          return console.error(
            `${index}. Error creating or getting category data.`,
          )
        }
        mem.set(post.name, response.data.data.documentId)
      }
      
      index++
    }
  } catch (e) {
    console.log(e.response)
  }
}

const spcode = (code) => {
  return code.split(".").length - 1
}

export const getAllPosts = async () => {
  const categories = [
    1, 280, 337, 354, 376, 409, 426, 446, 455, 28, 51, 79, 116, 130, 156, 183,
    198, 206, 264, 1040,
  ]
  
  const sortedCategories = [...categories].sort((a, b) => a - b)
  
  const secondItemIndex = sortedCategories.findIndex((category) => {
    return category === categories[1]
  })
  const secondItemGap = [
    sortedCategories[secondItemIndex],
    sortedCategories?.[secondItemIndex + 1]
      ? sortedCategories?.[secondItemIndex + 1] - 1
      : sortedCategories?.[secondItemIndex],
  ]
  
  const params = {
    draw: "1",
    "order[0][column]": "1",
    "order[0][dir]": "asc",
    start: "0",
    length: "32770",
    categories: [
      1, 280, 337, 354, 376, 409, 426, 446, 455, 28, 51, 79, 116, 130, 156, 183,
      198, 206, 264,
    ],
  }
  
  const response = await axios.post("https://roszdravnadzor.gov.ru/ajax/services/mi_reesetr", {}, {
    params: new URLSearchParams(params),
  })
  
  return response.data.data.map((item) => {
    item.col2.label = parseLine(item.col2.label)
    return {
      code: item.col1.label,
      name: item.col3.label,
      description: item.col4.label,
      section: item.col2.label[0],
      section_name: item.col2.label[1],
      category: item.col2.label?.[2],
      category_name: item.col2.label?.[3],
    }
  })
}

const parseLine = (line) => {
  const regex =
    /(\d+)\.\s([^\n<]+)(?:\s*<br>&nbsp;&nbsp;(\d+\.\d+)\.\s([^\n<]+))?/
  
  const match = line.match(regex)
  if (match) {
    const [, id, name, subId, subName] = match
    const result = [id, name.trim()]
    if (subId && subName) {
      result.push(subId, subName.trim())
    }
    return result
  }
  return null
}
